...
async function synthesizeMiniMax(
  ...
  if (
    ...
    return { buffer: buf, mimeType: contentType || "audio/mpeg" };
  }
  ...
  try {
    ...
    if (res.ok && raw.length > 0) {
      return { buffer: new TextEncoder().encode(raw).buffer, mimeType: "audio/mpeg" };
    }
  ...
  const buffer = decodeAudioPayload(audioStr);
